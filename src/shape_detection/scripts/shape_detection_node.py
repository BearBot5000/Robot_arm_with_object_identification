...
class ShapeDetectionNode(Node):
    ...
    def annotate_shapes(self, image, shapes):
        for shape, cx, cy in shapes:
            cv2.putText(image, shape, (cx, cy), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)

    def combine_results(self, top_down_shapes, side_view_shapes):
        combined_shapes = []

        for top_shape, top_cx, top_cy in top_down_shapes:
            for side_shape, side_cx, side_cy in side_view_shapes:
                if top_shape == "sphere" and side_shape == "circle":
                    combined_shapes.append(("sphere", top_cx, top_cy))
                elif top_shape == "circle" and side_shape == "cylinder":
                    combined_shapes.append(("cylinder", top_cx, top_cy))
                elif top_shape == "cube" and side_shape == "cube":
                    combined_shapes.append(("cube", top_cx, top_cy))
                elif top_shape == "rectangle" and side_shape == "cube":
                    combined_shapes.append(("cube", top_cx, top_cy))
                else:
                    combined_shapes.append((top_shape, top_cx, top_cy))
